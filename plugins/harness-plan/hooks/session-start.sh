#!/usr/bin/env bash
# Harness v2 SessionStart hook.
# Injects a compact summary from .harness/session-summary.json and .harness/current-contract.json.
#
# Locates .harness/ anywhere within CLAUDE_PROJECT_DIR (up to depth 6) so
# nested layouts (.engineering/implementation/.harness, packages/*/.harness)
# are picked up when the session starts at the repo root.

set -euo pipefail

PROJECT_DIR="${CLAUDE_PROJECT_DIR:-.}"

HARNESS_DIR="$(find "$PROJECT_DIR" -maxdepth 6 -type d -name '.harness' 2>/dev/null | head -n1)"
if [ -z "$HARNESS_DIR" ]; then
  exit 0
fi

CAMPAIGN_FILE="${HARNESS_DIR}/campaign.json"
SUMMARY_FILE="${HARNESS_DIR}/session-summary.json"
CONTRACT_FILE="${HARNESS_DIR}/current-contract.json"
AUTODRIVE_FILE="${HARNESS_DIR}/autodrive.json"

if [ ! -f "$CAMPAIGN_FILE" ]; then
  exit 0
fi

python3 - "$CAMPAIGN_FILE" "$SUMMARY_FILE" "$CONTRACT_FILE" "$AUTODRIVE_FILE" <<'PY'
import json
import sys
from pathlib import Path

campaign_path = Path(sys.argv[1])
summary_path = Path(sys.argv[2])
contract_path = Path(sys.argv[3])
autodrive_path = Path(sys.argv[4])

def load_json(path):
    if not path.exists():
        return None
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        return None

campaign = load_json(campaign_path)
if not campaign or not campaign.get("goal"):
    raise SystemExit(0)

summary = load_json(summary_path) or {}
contract = load_json(contract_path) or {}
autodrive = load_json(autodrive_path) or {}
counts = summary.get("progress_counts") or {}
review_policy = contract.get("review_policy") or campaign.get("default_review_policy", "selftest")
resume_steps = summary.get("resume_steps") or []
next_step = resume_steps[0] if resume_steps else "Run harness_summary.py to get the latest resume step."
known_failures = summary.get("known_failures") or []
env_status = summary.get("environment_status") or campaign.get("baseline_status", "unknown")

autodrive_enabled = bool(autodrive.get("enabled"))
autodrive_phase = autodrive.get("phase", "feature") if autodrive_enabled else None

lines = []
if autodrive_enabled and autodrive_phase == "feature":
    lines.extend([
        "[harness] AUTODRIVE MODE ACTIVE — read this first.",
        "Contract for this session (non-negotiable):",
        "  1. Implement exactly ONE feature, using the harness-plan skill to resume.",
        "  2. When that feature transitions to done, run `git add -u && git add .harness/ && git commit -m \"feat(harness): complete F0XX - <title>\"`.",
        "  3. End your response immediately after the commit. DO NOT pick another feature.",
        "  4. Never call AskUserQuestion — no human is present. If blocked, run:",
        "       python3 ${CLAUDE_PLUGIN_ROOT}/skills/harness-plan/scripts/harness_autodrive.py --project-root . --fail --reason \"<reason>\"",
        "     then end the response.",
        "  5. Do not run destructive scripts (reset, archive, force-push).",
        f"Autodrive iteration: {autodrive.get('iteration', 0)}/{autodrive.get('max_iterations', 20)}",
        "",
    ])
elif autodrive_enabled and autodrive_phase == "review":
    lines.extend([
        "[harness] AUTODRIVE REVIEW PHASE — you are the final-review session.",
        "Follow the review protocol the Stop hook handed you. Do not pick features. Mark review done and end.",
        "",
    ])

lines.extend([
    "[harness] Active campaign detected.",
    f"Goal: {summary.get('campaign_goal') or campaign.get('goal', '')}",
    (
        "Progress: "
        f"{counts.get('completed', campaign.get('completed_features', 0))}/"
        f"{counts.get('total', campaign.get('total_features', 0))} done"
    ),
    f"Current feature: {summary.get('current_feature') or campaign.get('current_feature') or 'none'}",
    f"Review policy: {review_policy}",
    f"Environment: {env_status}",
    f"Last session: {summary.get('last_session_date') or campaign.get('last_session_date') or 'unknown'}",
    f"Next: {next_step}",
])

handoff = summary.get("handoff_reason")
if handoff:
    lines.append(f"Previous session ended: {handoff}")

if env_status == "failing":
    lines.append("WARNING: Baseline environment is FAILING. Run bootstrap/setup before continuing.")

if known_failures:
    lines.append("Known failures:")
    for failure in known_failures[:5]:
        lines.append(f"  - {failure}")

open_issues = summary.get("open_issues") or []
if open_issues:
    lines.append("Open issues from last checkpoint:")
    for issue in open_issues[:5]:
        lines.append(f"  - {issue}")

last_failure = summary.get("last_selftest_failure")
if last_failure and isinstance(last_failure, dict):
    cmd = last_failure.get("command", "unknown command")
    err = last_failure.get("error_summary", "no details")
    lines.append(f"Last selftest failure: {cmd} — {err}")

context = "\n".join(lines)

print(json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "SessionStart",
        "additionalContext": context,
    }
}))
PY
